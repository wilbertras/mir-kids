...
def get_power(temp, a):
    inband = get_bp(theta, bp_mph, plot=True)
    bb = planck_wl(wl, np.array(temp).reshape((-1,1)))
    radiance = theta*bb / Eph
...
theta = bp185**5*nd1*nd3*znse**2*A*omegaBF
bp_mph=1e-19

# counts_vs_temp(dir, temps)
fit_efficiency(r'figures\Nph_18.5um_KID5_113dBm_40s.npy', fit_idx=[4,-2], xlim=[0,225], color='o', title='$18.5$ $\mu$m')
