...
from .data import MKIDDataException, SweepData, SweepFitData, FixedData, FixedFitData
from .peak_search import search_peak
from . import fitters
from . import fit
...
class KidFitResult(object):
    ...
    def rewind_data(self, d):
        if hasattr(d, 'info'):
            ...
            info = dict()
        if isinstance(d, FixedData):
            ampl, phase = self.rewound_ampl_phase(d.frequency, d.iq)
            return FixedFitData(d.frequency, d.t, ampl, phase, info=info)
        if isinstance(d, SweepData):
            ampl, phase = self.rewound_ampl_phase(d.x, d.iq)
            return SweepFitData(d.x, ampl, phase, info=info)
        else:
...
def fit_onepeak(data, fc, errors = None, nfwhm = 3, fitter = 'gaolinbg', Q_search=10000, plot=False):
    ...
    **fitter** supports:
    ...
    if nfwhm < 0:
        ...
        pdict = search_peak(data, fc=fc, Q_search=Q_search)
