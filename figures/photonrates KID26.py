...
from scipy.signal import find_peaks, peak_widths, fftconvolve, medfilt
...
def count_pulses(dir, kid, pread, nr_req_files, filter, lifetime, mph, mpp, coord='circle', response='phase', file_type='vis'):
    ...
    locs, _ = f.find_pks(medsignal, mph[0], mpp, window)
...
def counts_vs_temp(dir, temps):
    ...
    Nph = np.array(Nph)
    np.save('figures/Nph_%sum_KID%d_%ddBm_%ds_9std.npy' % (str(name), kid, pread, time), np.vstack((temps, Nph)))
    plt.show()
...
def fit_efficiency(path, fit_idx, xlim, ylim, color, title):
    data = np.load(path)
    temps = data[0,:]
    Nph = data[1,:]
    absorbed_rates = Nph
    ...
    yerr = np.sqrt(Nph)
    ax.errorbar(temps[:fit_idx[1]], absorbed_rates[:fit_idx[1]], yerr=yerr[:fit_idx[1]], linestyle='None', ecolor=color, elinewidth=1, marker='p', markerfacecolor=color, markeredgecolor=color, linewidth=2, label='%s photons' % title)
    ...
    # print('Background rate: %.1f +- %.3f mHz' % (Nbg*1e3, std_Nbg*1e3))
    Ndark = 20.4e-3
    ax.axhline(Ndark, ls='-.', c='k', label='$N_\mathrm{dark}^\mathrm{18.5 \mu m}$')
    ax.set_ylim(ylim)
    ax.set_xlim(xlim)
...
lifetime = 70
nr_req_files = 40
mph = np.array([0.046*9, 3])
mpp = mph[0] 
dir = r'D:\Data\LT218Chip1_BF_20240116_MIR18_5\KID5_all_temps\%dK'
dir = r'D:\Data\LT218Chip1_BF_20240116_MIR18_5\12KIDs_185um_BB%dK\TD_Power'
temps = [3,30,50,85,100,135,142,150,160,180]
Eph = sc.h*sc.c/(name*1e-6)
theta = bp185**5*nd1*nd3*znse**2*si*A*omegaBF
bp_mph=1e-19

# counts_vs_temp(dir, temps)
fit_efficiency(r'figures\Nph_18.5um_KID25_117dBm_40s_9std.npy', fit_idx=[2, None], xlim=[0,200], ylim=[1e-2, 1e2], color='o', title='$18.5$ $\mu$m')
...
# lifetime = 75
# nr_req_files = 40
# mph = np.array([6*0.068, 3])
# mpp = mph[0] 
# dir = r'Z:\KIDonSun\experiments\Entropy ADR\LT218Chip1_ADR_20240503_MIR24\12KIDs_3Pread_TD40s_1MHz_BB%dK\TD_Power'
# temps = [3,16,18,20,22,24,26,28]
# Eph = sc.h*sc.c/(name*1e-6)
# theta = bp25**3*lp*sp_a**3*sp_b*si*A*omegaADR
# bp_mph=9e-9
# bp_mph=2e-8
# # bp_mph=4e-10

# # counts_vs_temp(dir, temps)
# fit_efficiency(r'figures\Nph_25um_KID24_115dBm_40s.npy', fit_idx=[3, None], xlim=[0,30], ylim=[1e-2, 1e3], color='p', title='$25$ $\mu$m')
