...
fig, axes = plt.subplot_mosaic('a', figsize=(6,4), sharex=True, sharey=False, constrained_layout=True)
...
ax.semilogx(freqs[1:],  10*np.log10(dxx[1:]*rc_dark**2), label='dark BF', c='k', lw=width)
ax.semilogx(freqs[1:],  10*np.log10(nxx[1:]*rc_noise**2), label='3.8 $\mu$m, monochromator off', c='b', lw=width)
...
ax.semilogx(freqs[1:],  10*np.log10(nxx[1:]*rc_noise**2), label='8.5 $\mu$m, monochromator off', c='y', lw=width)
ax.set_ylabel(ylabel_psd)
# ax.set_ylim(ylim_psd)
...
ax.semilogx(freqs[1:],  10*np.log10(nxx[1:]*rc_noise**2), label='18.5 $\mu$m, $T_{bb}=3\ K$', c='o', lw=width)
ax.set_ylabel(ylabel_psd)
ax.set_ylim(ylim_psd)
...
# filter = False
# lifetime = 200
# noise, rc_noise = get_filtered_response(path_off, kid, pread, nr_req_files, filter, lifetime, file='KID5_113dBm_Tchip0.13_TDmed_TmK3255')
...
# ax.semilogx(freqs[1:],  10*np.log10(nxx[1:]*rc_noise**2)-2, label='25 $\mu$m, $T_{bb}=3\ K$', c='p')
# ax.semilogx(freqs[1:],  10*np.log10(dxx[1:]*rc_noise**2)-2, label='dark ADR', c='k')
# ax.set_ylabel(ylabel_psd)
# ax.set_xlabel(xlabel_psd)
