...
# --------------------------------------/ KHALILSWENSONMODEL IMPORTEREN\-------------
from Khalil import KhalilModel_magspace, KhalilSwensonModel
...
# --------------------------------------------------------------/ NIEUWE ARGUMENTEN                       \--------    
def loop_over_S21_files(path, kid=None, pread=None, plot=False, model=None, method=None, dw_low_power=None):
    if not kid:
        kid = '*'
    else:
        kid = str(kid)
    ...
    for i, file_path in enumerate(filenames):
        ...
        for temperature, df in data_by_temperature.items():
            ...
            if (np.min(frequencies) != np.max(frequencies)):
                ...
                # here we start fitting the data --------/ NIEUWE ARGUMENTEN         \-------
                result = Fit_S21(frequencies, s21_values, model, method, dw_low_power)
...
# ---------------------/ NIEUWE ARGUMENTEN                       \----------
def Fit_S21(f, S21_dB, model=None, method=None, dw_low_power=None):
